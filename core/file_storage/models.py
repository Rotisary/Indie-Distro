...
class FileModel(BaseModelMixin):
    ...
    file_width = models.IntegerField(_("Width of File"), null=True, blank=True)
    file_height = models.IntegerField(_("Height of File"), null=True, blank=True)
    file_size = models.BigIntegerField(_("File Size in bytes"), null=True, blank=True)
...
class FileProcessingJob(BaseModelMixin):
    ...
    source_key = models.CharField(max_length=1024, verbose_name=_("File Source Key"))
    ...
    stages = JSONField(
        default=dict, blank=True, null=True, verbose_name=_("Per stage details")
    )
    metadata = JSONField(
        default=dict,
        blank=True,
    ...
    thumbnails = JSONField(
        default=dict, blank=True, null=True, help_text=_("list of thumbnail keys")
    )
    audio = JSONField(
        default=dict, blank=True, null=True, help_text=_("audio related outputs")
    )
    ...
    def mark_stage(self, stage: str, data: dict = None):
        ...
        self.stages = s
        self.save(
            update_fields=["current_stage", "status", "stages", "date_last_modified"]
        )
