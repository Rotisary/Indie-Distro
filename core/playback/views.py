...
class RetrieveFilmPlaybackURL(views.APIView):
    ...
    def post(self, request):
        ...
        user = request.user
        purchase, cookie_value, expires_at, cookie_path = AccessUtils.return_stream_cookie_for_film(
            film_id, user
        )
...
class RetrieveShortPlaybackURL(views.APIView):
    ...
    @IdempotencyDecorator.make_endpoint_idempotent(ttl=60, namespace="short_playback_url")
    def post(self, request):
        ...
        short, cookie_value, expires_at, cookie_path = AccessUtils.return_stream_cookie_for_short(
            short_id, user
        )
...
class RefreshShortPlaybackCookie(views.APIView):
    ...
    @IdempotencyDecorator.make_endpoint_idempotent(ttl=60, namespace="short_playback_refresh")
    def post(self, request):
        serializer = ShortIDSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        short_id = serializer.validated_data["short_id"]
