...
class FilmFilter(FilterSet):
    ...
    language = filters.CharFilter(
        field_name="language", label="Filter by the language of the film"
    )
    release_date__gte = filters.DateFilter(
        field_name="release_date", lookup_expr="gte", label="Return recent releases"
    )
    release_date__lte = filters.DateFilter(
        field_name="release_date", lookup_expr="lte", label="Return upcoming releases"
    )
...
class ShortFilter(FilterSet):
    owner = filters.NumberFilter(field_name="owner__id", label="Filter by owner ID")
    film = filters.NumberFilter(
        field_name="film__id", label="Filter by associated film ID"
    )
    type = filters.ChoiceFilter(
    ...
    search = filters.CharFilter(
        method="filter_search", label="Search in caption and tags"
    )

    def filter_search(self, queryset, name, value):
        return queryset.filter(Q(caption__icontains=value) | Q(tags__icontains=value))
